...
const openSharePreview = (data: LogEntry[]) => {
  ...
  return (
    <div key={dateUpdateTrigger} className="min-h-screen bg-gradient-to-br from-jasmine-50 via-white to-jasmine-50">
      <div className="max-w-7xl mx-auto px-6 py-12 space-y-10">
        ...
        <div className="text-center space-y-4">
          <div className="flex justify-center mb-4">
            <img 
              src="/logo.png" 
              alt="OREA 85 Logo" 
              className="h-20 w-20 object-cover rounded-full border-4 border-jasmine-200 shadow-lg hover:scale-105 hover:shadow-xl transition-all duration-300 ease-in-out"
            />
          </div>
          <h1 className="text-5xl font-light tracking-tight text-jasmine-900">
            Logbook Komisi
          </h1>
          <p className="text-lg text-jasmine-700 font-light">
            Management system for treatment commissions
          </p>
          <p className="text-sm text-jasmine-600 font-light">
          ...
          <CardHeader className="pb-6">
            <CardTitle className="text-xl font-medium luxury-text flex items-center gap-3">
              <div className="icon-wrapper">
                <Plus className="w-4 h-4 text-jasmine-600" />
              </div>
              Add New Entry
            </CardTitle>
            {/* Digital Clock */}
            <div className="mt-4 text-center">
              <div className="text-3xl font-light text-jasmine-900 tracking-wider font-mono">
                {currentTimeWIB}
              </div>
              <div className="text-sm text-jasmine-600 font-light">
                {getWIBDateString()}
              </div>
            </div>
          </CardHeader>
          <CardContent>
            <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-5 gap-4">
              <div className="md:col-span-2 space-y-2">
                <Label htmlFor="treatment" className="text-sm font-medium text-jasmine-700">
                  Treatment Name
                </Label>
                <div className="relative" ref={popupRef}>
                  <button
                    ...
                    className="w-full border-jasmine-200 bg-white text-jasmine-900 focus:border-jasmine-400 focus:ring-jasmine-400/20 rounded-lg px-3 py-2 text-left flex items-center justify-between hover:border-jasmine-300 transition-colors"
                  >
                    <span className={selectedTreatment ? 'text-jasmine-900' : 'text-jasmine-400'}>
                      {selectedTreatment || 'Select treatment'}
                    </span>
                    <ChevronDown className={`w-4 h-4 text-jasmine-400 transition-transform ${isTreatmentPopupOpen ? 'rotate-180' : ''}`} />
                  </button>
                  ...
                  {isTreatmentPopupOpen && (
                    <>
                      ...
                      <div className="treatment-dropdown-desktop popup-animation">
                        <div className="p-3 border-b border-jasmine-200 flex items-center justify-between">
                          <div className="relative flex-1 mr-3">
                            <Search className="absolute left-3 top-1/2 transform -translate-y-1/2 w-4 h-4 text-jasmine-400" />
                            <input
                              type="text"
                              placeholder="Search treatments..."
                              value={treatmentSearch}
                              onChange={(e) => setTreatmentSearch(e.target.value)}
                              className="w-full pl-10 pr-4 py-2 border border-jasmine-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-jasmine-400 focus:border-transparent text-sm"
                            />
                          </div>
                          <button
                            onClick={handlePopupClose}
                            className="p-2 hover:bg-jasmine-100 rounded-lg transition-colors md:hidden"
                          >
                            <X className="w-4 h-4 text-jasmine-400" />
                          </button>
                        </div>
                        <div className="max-h-48 overflow-y-auto">
                          {filteredTreatments.length === 0 ? (
                            <div className="px-3 py-4 text-center text-jasmine-500 text-sm">
                              No treatments found
                            </div>
                          ) : (
                            filteredTreatments.map((treatment) => (
                              ...
                              >
                                <div className="flex items-center justify-between">
                                  <span className="text-sm text-jasmine-900">
                                    {treatment.name}
                                  </span>
                                  <span className="text-xs text-jasmine-500">
        ...
        {/* Footer Copyright */}
        <div className="text-center py-6 border-t border-jasmine-200">
          <div className="space-y-3">
            <p className="text-sm text-jasmine-400 font-light">
              © Developed by OREA 85 - {format(new Date(), 'yyyy')} | All rights reserved
            </p>
            <div className="flex items-center justify-center gap-2">
              <p className="text-sm text-jasmine-600 font-bold uppercase tracking-wider">
